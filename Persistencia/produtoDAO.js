...
export default class ProdutoDAO {
    ...
    async incluir(produto) {
        if (produto instanceof Produto) {
            ...
            const sql = `INSERT INTO produto(prod_descricao,prod_precoCusto,prod_precoVenda,prod_qtdEstoque,prod_urlImagem,
                        prod_dataValidade, fk_codigo_cat)
    ...
    async alterar(produto) {
        if (produto instanceof Produto) {
            const conexao = await conectar();
            const sql = `UPDATE produto SET prod_descricao=?,prod_precoCusto=?,prod_precoVenda=?,prod_qtdEstoque=?,prod_urlImagem=?,
                        prod_dataValidade=str_to_date(?,'%d/%m/%Y'), fk_codigo_cat = ?
                WHERE prod_codigo = ?
            `;
            let parametros = [
                produto.descricao,
    ...
    async consultar(termo) {
        ...
        if (isNaN(parseInt(termo))) {
            ...
            sql = "SELECT * FROM produto p INNER JOIN categoria c ON p.fk_codigo_cat = c.codigo WHERE codigo = ?"
            parametros = [termo];
        }
